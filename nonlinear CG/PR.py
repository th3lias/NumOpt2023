...
class PR:
    ...
    def beta(self, x, x_old):
        return (self.df(self.f, x).T @ (self.df(self.f, x) - self.df(self.f, x_old)) / np.linalg.norm(self.df(self.f, x_old)) ** 2)

    # main function
if __name__ == "__main__":
    # pass rosennbrock function with point (1.2, 1.2)
    pr = PR(lambda x: 100 * (x[1] - x[0] ** 2) ** 2 + (1 - x[0]) ** 2,
            lambda f, x: np.array([-400 * x[0] * (x[1] - x[0] ** 2) - 2 * (1 - x[0]), 200 * (x[1] - x[0] ** 2)]),
    ...
    pr = PR(lambda x: 150 * (x[0] * x[1]) ** 2 + (0.5 * x[0] + 2 * x[1] - 2) ** 2, lambda f, x: np.array(
    ...
    pr = PR(f, approximate_gradient, np.array([1.2, 1.2]), 1e-6, np.array([1, 1]))
    results, no_iter, x, grad_norm, distance_solution = pr.run()
    print("Rosennbrock function with point (1.2, 1.2)")
    print("Number of iterations: ", no_iter)
    print("Solution: ", x)
    ...
    print("#############################################")
    # pass rosennbrock function with point (0.2, 0.8)
    pr = PR(lambda x: 100 * (x[1] - x[0] ** 2) ** 2 + (1 - x[0]) ** 2, approximate_gradient, np.array([0.2, 0.8]), 1e-6,
            np.array([1, 1]))
    results, no_iter, x, grad_norm, distance_solution = pr.run()
    print("Rosennbrock function with point (0.2, 0.8)")
    print("Number of iterations: ", no_iter)
    ...
    print("#############################################")
    # pass f(x) = 150 * (x1*x2) ** 2 + (0.5 * x1 + 2 * x2 - 2) ** 2 with point (-0.2, 1.2)
    pr = PR(lambda x: 150 * (x[0] * x[1]) ** 2 + (0.5 * x[0] + 2 * x[1] - 2) ** 2, approximate_gradient,
            np.array([-0.2, 1.2]), 1e-6, np.array([0, 1]))
    results, no_iter, x, grad_norm, distance_solution = pr.run()
    print("f(x) = 150 * (x1*x2) ** 2 + (0.5 * x1 + 2 * 2* x2 - 2) ** 2 with point (-0.2, 1.2)")
    print("Number of iterations: ", no_iter)
    ...
    print("#############################################")
    # pass f(x) = 150 * (x1*x2) ** 2 + (0.5 * x1 + 2 * x2 - 2) ** 2 with point (3.8, 0.1)
    pr = PR(lambda x: 150 * (x[0] * x[1]) ** 2 + (0.5 * x[0] + 2 * x[1] - 2) ** 2, approximate_gradient, np.array([3.8, 0.1]), 1e-6, np.array([4, 0]))
    results, no_iter, x, grad_norm, distance_solution = pr.run()
    print("f(x) = 150 * (x1*x2) ** 2 + (0.5 * x1 + 2 * 2* x2 - 2) ** 2 with point (3.8, 0.1)")
    print("Number of iterations: ", no_iter)
    print("Solution: ", x)
    ...
    pr = PR(lambda x: 150 * (x[0] * x[1]) ** 2 + (0.5 * x[0] + 2 * x[1] - 2) ** 2, approximate_gradient, np.array([1.9, 0.6]), 1e-6, np.array([4, 0]))
    results, no_iter, x, grad_norm, distance_solution = pr.run()
    print("f(x) = 150 * (x1*x2) ** 2 + (0.5 * x1 + 2 * 2* x2 - 2) ** 2 with point (1.9, 0.6)")
    print("Number of iterations: ", no_iter)
    print("Solution: ", x)
